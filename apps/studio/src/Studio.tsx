  ...
  return <div className={`studio theme-${theme}`}>
    ...
    <main id="workspace" className="workspace" aria-busy={!!busy}>
      <aside className="panel sources" aria-labelledby="sources-title"><p className="eyebrow">01 · SOURCES</p><h2 id="sources-title">The material</h2>
        ...
        <label className="unavailable">Select a local file <input type="file" accept=".png,.jpg,.jpeg,.webp,.txt" disabled={!paired || frozen} aria-describedby="import-status" onChange={e => setChosenFile(e.target.files?.[0] ?? null)} /></label>
        <p id="import-status" className="fine">PNG, JPEG, WebP or UTF-8 text, up to 50 MB. Selection alone sends nothing. Text is preserved and creates a bounded visual excerpt; audio, video and PDF imports are not qualified yet.</p>
