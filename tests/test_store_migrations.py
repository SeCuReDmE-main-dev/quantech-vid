...
from quantech_vid.store_migrations import LEGACY_ORIGINALS_SQL, allow_glb_originals
...
def test_failed_migration_rolls_back_without_losing_originals(tmp_path):
    path, db, original = legacy(tmp_path)
    try:
        db.execute("CREATE TABLE source_originals_glb_migration(occupied TEXT)")
